...
$(function (){
    ...
    $.get("route/isFavorite",{rid:rid},function (flag){
       if(flag){
           ...
           $("#favorite").attr("onclick","delFavorite()");
           $("#favorite").html('<i class=\"glyphicon glyphicon-heart-empty\"></i>取消收藏');
       }else {
           $("#favorite").removeAttr("onclick");
           $("#favorite").attr("onclick","addFavorite()");
           $("#favorite").html('<i class=\"glyphicon glyphicon-heart-empty\"></i>点击收藏');
       }
    });
...
function delFavorite(){
    $.get("user/findOne",{},function (user){
        //如果用户登入则刷新addFavorite
        if(user){
            $.get("route/delFavorite",{rid:rid},function (){
                location.reload();
            });
        }else {
            self.location="http://localhost:8080/travel/login.html";
